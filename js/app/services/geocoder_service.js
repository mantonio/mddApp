    ...
    var executeNext = function() {
        ...
        geocoder.geocode({
            ...
            if (status === google.maps.GeocoderStatus.OK) {
                ...
                locations[restaurant.address] = parsedResult;

                $localStorage.locations = JSON.stringify(locations);

                queue.shift();
                restaurant.d.resolve(parsedResult);

            } else if (status === google.maps.GeocoderStatus.ZERO_RESULTS) {
                queue.shift();
                restaurant.d.reject({
                    type: 'zero',
                    message: 'Zero results for geocoding address ' + restaurant.address
                });
            } else if (status === google.maps.GeocoderStatus.OVER_QUERY_LIMIT) {
                if (restaurant.executedAfterPause) {
                    queue.shift();
                    restaurant.d.reject({
                        type: 'busy',
                        message: 'Geocoding server is busy can not process address ' + restaurant.address
                    });
                }
                ...
                restaurant.d.reject({
                    type: 'denied',
                    message: 'Request denied for geocoding address ' + restaurant.address
                });
            } else {
                queue.shift();
                restaurant.d.reject({
                    type: 'invalid',
                    message: 'Invalid request for geocoding: status=' + status + ', address=' + restaurant.address
                });
            }

            if (queue.length) {
                if (status === google.maps.GeocoderStatus.OVER_QUERY_LIMIT) {
                    var nextrestaurant = queue[0];
                    nextrestaurant.executedAfterPause = true;
